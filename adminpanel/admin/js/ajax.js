...
// Add Exam 
$(document).on("submit","#addExamCatFrm" , function(){
  $.post("query/addExamCatExe.php", $(this).serialize() , function(data){
    if(data.res == "noSelectedCourse")
   {
      Swal.fire(
          'No Course',
     ...
     else if(data.res == "exist")
    {
      Swal.fire(
        'Already Exist',
        data.CategoryTitle.toUpperCase() + '<br>Already Exist',
        'error'
      )
    }
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
        data.CategoryTitle.toUpperCase() + '<br>Successfully Added',
        'success'
      )
          $('#addExamCatFrm')[0].reset();
          // $('#course_name').val("");
          refreshDiv();
    }
  },'json')
